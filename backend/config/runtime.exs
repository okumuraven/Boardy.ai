...
# drives five scheduled sweeps: the Bizi Buddy System's weekly check-in
...
# this to daily once that delay moves to days post-hackathon), the
# Bizi verification "documents still outstanding" nudge daily at 10am UTC
# (Phase E, bizi_verification_build_plan.md), and a daily retry for any
# interview (voice or chat) still stuck in the raw-transcript fallback
# after a failed Gemini summarization.
config :vokazi, Oban,
  engine: Oban.Engines.Basic,
  repo: Vokazi.Repo,
  queues: [push: 5, buddy_checkins: 1, mailers: 5, bizi_reminders: 1, interview_retries: 1],
  plugins: [
    {Oban.Plugins.Cron,
     crontab: [
       ...
       {"30 8 * * *", Vokazi.Interviews.RetryUnsummarizedWorker},
